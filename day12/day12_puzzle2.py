...
for path in paths:
    stops = path[1:-1]
    # Try all different possible stops
    for stop in [stops[:i] for i in range(1, len(stops) + 1)]:
        # Keeping list of visited
        seen = set([i for i in stop if i.islower()])
        # Now try with each seen being allowed to be visited twice
        for spot in seen:
            for p in findPathsVisited(stop[-1], seen.difference({spot}).union({'start'})):
                double_paths.add(tuple(['start'] + list(stop[:-1]) + list(p)))
print(len(double_paths.union(paths)))