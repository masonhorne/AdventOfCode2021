...
seq, rule_text = sys.stdin.read().split('\n\n')
rules = {}
...
for _ in range(40):
    temp = polymer.copy()
    for val in (compound for (compound, count) in polymer.items()):
        first_polymer = val[:1] + rules[val]
        second_polymer = rules[val] + val[1:]
        temp[first_polymer] += polymer[val]
        temp[second_polymer] += polymer[val]
        temp[val] -= polymer[val]
        total[rules[val]] += polymer[val]
    polymer = temp.copy()
count = collections.Counter(total)
# Add another counter to remove elements that appear 0 times
