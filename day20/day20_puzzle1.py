...
def update_image(points, algo, default='.'):
    new_points = collections.defaultdict(lambda: default)
...
algorithm, data = sys.stdin.read().split('\n\n')
data = [list(line) for line in data.splitlines()]
points = create_points(data)

for i in range(2):
    if i % 2 == 0 or algorithm[0] == '.':
        default = algorithm[0]
    else:
        default = algorithm[-1]
    points = update_image(points, algorithm, default)
print(sum(1 for point in points if points[point] == '#'))