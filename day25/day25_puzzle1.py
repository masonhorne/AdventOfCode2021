...
Boost strength due to matching signal phase: 49 stars
Only 1 stars to go.
...
# Helper function for debugging that displays map
def show_state(south, east):
...
step = 0
moved = True
east = set([(i, j) for i in range(width) for j in range(height) if data[j][i] == '>'])
south = set([(i, j) for i in range(width) for j in range(height) if data[j][i] == 'v'])
while moved:
    show_state(east, south)
    moved = False
    step += 1
    new_east = set()
    new_south = set()
    for cucumber in east:
        new_cucumber = ((cucumber[0] + 1) % width, cucumber[1])
    ...
    for cucumber in south:
        new_cucumber = (cucumber[0], (cucumber[1] + 1) % height)
